...
- (void)viewDidLoad
{
    ...
    btnNext.frame=CGRectMake(self.view.bounds.size.width*2/3, 0, self.view.bounds.size.width/3, 44);
    [btnNext setTitle:@"下一步" forState:UIControlStateNormal];
    [btnNext setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    btnNext.titleLabel.font=[UIFont fontWithName:DeviceFontName size:DeviceFontSize];
    btnNext.showsTouchWhenHighlighted = YES;  //指定按钮被按下时发光
    [btnNext setTitleColor:[UIColor colorFromHexRGB:@"4a7ebb"] forState:UIControlStateHighlighted];
    [btnNext addTarget:self action:@selector(buttonNextClick) forControlEvents:UIControlEventTouchUpInside];
    [buttons addSubview:btnNext];
    ...
    [self uploadImageWithId:@"" completed:^(NSString *fileName) {
        ...
        [self.serviceHelper asynService:args success:^(ServiceResult *result) {
            ...
            if (result.hasSuccess) {
                ...
                if ([[dic objectForKey:@"Result"] length]>2) {
                    boo=YES;
